...
# vis
from astropy.visualization import ZScaleInterval,LinearStretch,PercentileInterval
vis_transform = LinearStretch() + PercentileInterval(99.8)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                _gt = visuals['gt'].squeeze(0).float().detach().cpu().numpy().transpose(1, 2, 0)
                _lq = visuals['lq'].squeeze(0).float().detach().cpu().numpy().transpose(1, 2, 0)
                _sr = visuals['result'].squeeze(0).float().detach().cpu().numpy().transpose(1, 2, 0)
                vis_png = np.hstack([vis_transform(_gt),
                                        vis_transform(_lq),
                                        vis_transform(_sr)]) * 255
                vis_npy = np.hstack([(_gt),(_lq),(_sr)])
                ...
                cv2.imwrite(save_img_path, vis_png.astype(np.uint8))
                np.save(save_img_path.replace(".png",".npy"), vis_npy)
