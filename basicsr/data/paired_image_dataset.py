...
class Dataset_NPY(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.paths = sorted(glob("{}/*.npy".format(self.gt_folder)))
    ...
    def __getitem__(self, index):
        ...
        img_lq = torch.from_numpy(np.ascontiguousarray(img_lq.transpose(2, 0, 1)))
        img_gt = torch.from_numpy(np.ascontiguousarray(img_gt.transpose(2, 0, 1)))
...
class LOLBlurDatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_lq = np.array(load_img(data_filenames[index1-1]))
        img_gt = np.array(load_img(data_filenames2[index1-1]))
        seed = random.randint(1, 1000000)
        ...
        return {
            'lq': img_lq,
            'gt': img_gt,
            'lq_path': data_filenames[index1-1],
            'gt_path': data_filenames2[index1-1]
        }
